(() => {
    ...
    if (sidebar) {
      sidebar.innerHTML = real.featured.slice(0, 4).map(user => `
        <div class="followed-channel" data-featured-platform="${escapeHTML(user.platform)}" data-featured-name="${escapeHTML(user.username)}">
          <img src="${escapeHTML(user.avatar)}" class="followed-avatar" alt="">
  ...
  function renderRealSuggested() {
    ...
    container.innerHTML = cards.map(user => `
      <div class="suggested-card featured-rotating-card">
        <img src="${escapeHTML(user.avatar)}" alt=""><div class="name">${escapeHTML(user.name)}</div>
  ...
  async function searchGlobal(query, platform) {
    try {
      ...
      return (payload.items || []).filter(item => item.platform === platform).map(item => ({
        ...
        avatar: item.avatar, thumbnail: item.avatar, live: item.live, category: item.category, viewers: item.viewers || '—', url: item.url
      }));
    } catch { return []; }
  }
  ...
  window.searchKick = async query => {
    try { const payload = await api(`/api/channel/kick/${encodeURIComponent(query)}`); const item = payload.channel; return [{ id: item.id, name: item.username, username: item.username, displayName: item.name, platform: 'kick', avatar: item.avatar, live: item.live, category: item.category, viewers: item.viewers, url: item.url }]; }
    catch { return []; }
  };
  ...
  window.loadBrowseCategories = async () => {
    ...
    try {
      ...
      content.innerHTML = items.length ? `<div class="browse-section-label">Explore ${escapeHTML(currentBrowsePlatform)} categories</div><div class="browse-category-grid">${items.map(category => `
        ...
        <div class="browse-category-name">${escapeHTML(category.name)}</div><div class="browse-category-viewers">${compact(category.watching)} watching</div></article>`).join('')}</div>` : renderBrowseEmpty('No real categories match this search.');
  ...
  window.selectCategory = async (name, image, id) => {
    currentBrowseCategory = { id: id || getBrowseCategories().find(item => item.name === name)?.id || '', name, image, watching: getBrowseCategories().find(item => item.name === name)?.watching || 0, followers: null, tags: [] };
  ...
  window.renderBrowseCategoryDetail = async () => {
    ...
    try {
      ...
      content.innerHTML = `<section class="category-detail"><div class="category-detail-top"><div class="category-detail-art">${currentBrowseCategory.image ? `<img src="${escapeHTML(currentBrowseCategory.image)}" alt="">` : ''}</div><div class="category-detail-info"><h2>${escapeHTML(currentBrowseCategory.name)}</h2><div class="category-detail-stats"><span>${compact(currentBrowseCategory.watching)} watching</span><span>Live platform data</span></div></div><button class="category-back-button" onclick="returnToBrowseCategories()"><i class="fa-solid fa-arrow-left"></i> Categories</button></div>
  ...
  window.showStreamHoverCard = async (stream, targetElement) => {
    ...
    const avatar = detail.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(detail.name || stream.displayName)}&background=17202b&color=eff6ff`;
    ...
    card.innerHTML = `<div class="hover-banner" style="background-image:url('${String(detail.banner || '').replace(/['()]/g, '')}')"></div><div class="hover-pfp-wrapper"><img src="${escapeHTML(avatar)}" class="hover-pfp" alt="">${detail.live ? '<div class="hover-live-badge">LIVE</div>' : ''}</div>
      <div class="hover-content"><div class="hover-username">${escapeHTML(detail.name || stream.displayName)}</div><div class="hover-followers">${detail.followers === null || detail.followers === undefined ? 'Follower count not exposed by this platform' : `${compact(detail.followers)} Followers`}</div>
